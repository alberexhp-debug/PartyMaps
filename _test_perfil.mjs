...
// en /mi-pagina, /consola/perfil y desde OTRAS cuentas (clave de mundo).
{
  ...
  const nombreInput = page.locator('input:not([type="file"])').first()
  ...
  // Personalización (31-08): foto y banner también para el TO
  ok(await page.getByText('Foto de perfil').count() > 0, 'el editor ofrece foto de perfil')
  await page.getByRole('button', { name: 'Violeta' }).click(); await page.waitForTimeout(300)
  ...
  ok(await page.locator('div[style*="linear-gradient(120deg"]').count() > 0, 'y el banner elegido corona su página pública')
