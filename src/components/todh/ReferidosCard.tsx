...
const NIVELES: { n: 1 | 3 | 5; premioK: ClaveI18n; emoji: string }[] = [
  { n: 1, premioK: 'ref.premio1', emoji: '👀' },
  { n: 3, premioK: 'ref.premio3', emoji: '🎮' },
  { n: 5, premioK: 'ref.premio5', emoji: '🏆' },
]
...
export function ReferidosCard() {
  ...
  const copiar = async () => {
    try {
      await navigator.clipboard.writeText(`https://torneum.app/r/${codigo}`)
      setCopiado(true)
  ...
  return (
    <div className="card-premium ring-grad p-4 relative overflow-hidden">
      ...
      <div className="relative flex items-center gap-2 mb-1">
        ...
        <p className="eyebrow eyebrow-muted">{tr('ref.titulo')}</p>
      </div>
      <p className="text-[13px] text-[#B8B8CC]">{tr('ref.textoA')} <strong className="text-white">{tr('ref.textoB')}</strong>.</p>
      ...
      <button onClick={copiar} className="mt-3 w-full flex items-center justify-between rounded-xl border border-dashed border-[#B6FF3A]/45 bg-[#B6FF3A]/6 px-4 h-12">
        ...
        <span className="inline-flex items-center gap-1.5 text-xs font-bold text-white">
          {copiado ? <><Check size={14} className="text-[#B6FF3A]" /> {tr('ref.copiado')}</> : <><Copy size={14} /> {tr('ref.copiarEnlace')}</>}
        </span>
      </button>
          ...
          return (
            <div key={n} className={`flex items-center gap-2.5 rounded-xl px-3 py-2 border ${canjeado ? 'bg-white/[0.03] border-white/8' : disponible ? 'bg-[#B6FF3A]/8 border-[#B6FF3A]/35' : 'bg-white/[0.03] border-white/8 opacity-60'}`}>
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-[13px] font-semibold ${canjeado ? 'text-[#8B8BA8] line-through' : 'text-white'}`}>{tr(premioK)}</p>
                <p className="text-[10px] text-[#8B8BA8]">{n} {n === 1 ? tr('ref.amigoJuegue') : tr('ref.amigosJueguen')}</p>
              </div>
              {canjeado
                ? <span className="text-[10px] font-bold text-[#8B8BA8] inline-flex items-center gap-1"><Check size={12} /> {tr('ref.canjeado')}</span>
                : disponible
                ? <button onClick={() => canjear(n)} className="h-7 px-2.5 rounded-lg bg-[#B6FF3A] text-[#0A0A0F] text-[11px] font-black">{tr('ref.canjear')}</button>
