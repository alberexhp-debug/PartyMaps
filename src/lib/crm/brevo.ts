...
export async function brevoSyncLista(apiKey: string, nombreLista: string, contactos: { tel: string; nombre: string }[]): Promise<{ ok: boolean; importados: number; error?: string }> {
  try {
    // 1) Carpeta (reutiliza la primera o crea "Torneum").
    ...
    if (!folderId) {
      const cf = await fetch(`${BASE}/contacts/folders`, { method: 'POST', headers: headers(apiKey), body: JSON.stringify({ name: 'Torneum' }) })
