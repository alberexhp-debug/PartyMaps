...
import { JUEGOS, LOCALES, plantillaDe, organizadorEfectivo, type TorneoSample, type Tier, type Local } from '@/lib/torneos/sample'
...
// Etiqueta traducida de cada cierre (el VALOR guardado sigue siendo el ES: es dato).
const CIERRE_CLAVE: Record<string, ClaveI18n> = {
  'Al empezar': 'ct.cierreAlEmpezar', '1 hora antes': 'ct.cierre1h', '1 día antes': 'ct.cierre1d', 'Manual': 'ct.cierreManual',
}
...
export default function CrearTorneoPage() {
  ...
  const ocultosAdmin = useDemoStore(s => s.juegosOcultos)
  // Identidad por cuenta: el torneo se publica a nombre de QUIEN entra (Lima
  // o un jugador aprobado como TO) — nada de asumir «lima».
  const orgId = useOrgId()
  const localSesion = useSesionStore(s => s.sesion?.localId)
  ...
  // Los desactivados por el admin no admiten torneos nuevos.
  const juegos = useMemo(() => Object.values(JUEGOS).filter(g => !ocultosAdmin.includes(g.id)), [juegosCustom, ocultosAdmin])
  const j = JUEGOS[juego]
  // La plantilla del juego dicta formatos sugeridos y sets por defecto.
  const plantilla = plantillaDe(juego)
  // Si la sesión trae un local asociado, arranca preseleccionado (derivado,
  // sin efecto): el TO puede cambiarlo con el picker.
  const sedeEfectivaId = sedeId ?? localSesion ?? null
  const sede = sedeEfectivaId ? LOCALES[sedeEfectivaId] : null
  const elegirJuego = (id: string) => { setJuego(id); setFormato(plantillaDe(id).formatos[0]) }
  ...
  return (
    <div className="relative min-h-screen pb-44 lg:pb-16 max-w-xl mx-auto lg:max-w-none lg:mx-0">
      ...
      <div className="flex items-center gap-3 px-4 pt-5 pb-3 safe-top sticky top-0 z-10 bg-[#0D0F15]/92 backdrop-blur-md border-b border-white/6">
        ...
        <div>
          <p className="text-[11px] text-[#8B8BA8] uppercase tracking-wider font-semibold">{tr('ct.consolaTO')}</p>
      ...
      {/* Fluido: el formulario crece hasta 880px pegado al rail y la vista previa
          se ancla a la derecha (justify-between reparte el espacio sobrante). */}
      <div className="lg:grid lg:grid-cols-[minmax(0,880px)_400px] lg:justify-between lg:gap-10 lg:px-8 lg:items-start lg:mt-6">
      ...
      <div className="px-5 pt-4 lg:px-0 lg:pt-0 lg:col-start-2 lg:row-start-1 lg:sticky lg:top-24">
          ...
          <div className="flex-1 p-3.5 min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              <span className="inline-flex items-center gap-1.5 px-2 h-6 rounded-full text-[10px] font-bold" style={{ background: `${j.color}1F`, color: j.color, border: `1px solid ${j.color}44` }}>
                <GameIcon juegoId={juego} size={12} /> {j.corto}
              </span>
            ...
            <p className="font-bold text-white text-display tracking-tight text-[15px] leading-snug truncate">{nombre || tr('ct.previewNombre')}</p>
            <div className="mt-1.5 flex items-center gap-2 text-[11px] text-[#A0A0B8] min-w-0">
              <span className="inline-flex items-center gap-1 text-white shrink-0"><Calendar size={11} className="text-[#B6FF3A]" /> {fecha ? new Date(fecha + 'T00:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }) : tr('adm.fecha')} · {hora}</span>
              <span className="text-[#3A3A4A]">·</span>
              <span className="inline-flex items-center gap-1 truncate">{sala === 'online' ? <Globe size={11} /> : <MapPin size={11} />} {sala === 'online' ? 'Online' : sede?.nombre || tr('ct.eligeSede')}</span>
            </div>
            <div className="mt-2.5 flex items-end justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between text-[10px] mb-1">
                  ...
                  <span className="font-semibold text-[#B6FF3A]">{tr('card.abierta')}</span>
                </div>
              ...
              <div className="text-right shrink-0">
                <p className="text-[8px] text-[#8B8BA8] uppercase tracking-[0.12em] font-bold">{precio > 0 ? tr('card.bote') : tr('card.entrada')}</p>
                <p className="text-[15px] font-bold text-white font-mono-num leading-none mt-0.5">{precio > 0 ? `${Math.round(plazas * precio * 0.8)}€` : tr('card.free')}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <button onClick={publicar} disabled={!nombre.trim()}
          className="hidden lg:flex mt-4 w-full h-13 py-3.5 rounded-2xl bg-[#B6FF3A] text-[#0A0A0F] font-bold text-[15px] shadow-[0_10px_30px_-8px_rgba(182,255,58,0.5)] items-center justify-center gap-2 disabled:opacity-50">
          {tr('ct.publicar')} <GameIcon juegoId={juego} size={15} color="#0A0A0F" /> {j.corto}
        </button>
      </div>
      ...
      <div className="px-5 pt-5 space-y-6 lg:px-0 lg:pt-0 lg:col-start-1 lg:row-start-1">
        ...
        <Section title={tr('ct.basico')}>
          <Field label={tr('ges.lblNombre')}>
            <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder={tr('ct.nombrePh')}
          ...
          <Field label={tr('ct.lblJuego')}>
            <div className="flex flex-wrap gap-2">
              {juegos.map(g => {
                const on = juego === g.id
                return (
                  <button key={g.id} onClick={() => elegirJuego(g.id)}
            ...
            {nuevoJuego && <NuevoJuegoForm onCerrar={() => setNuevoJuego(false)} />}
          </Field>
          <Field label={tr('ct.lblFormato')}>
            <input value={formato} onChange={e => setFormato(e.target.value)} placeholder={tr('ges.formatoPh')}
              className="w-full h-12 px-3.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-[#6B6B85] focus:border-[#B6FF3A]/60 outline-none transition-colors" />
            <div className="flex flex-wrap gap-2 mt-2">
              {plantilla.formatos.map(f => (
                <Chip key={f} on={formato === f} onClick={() => setFormato(f)}>{f}</Chip>
              ))}
            </div>
            <p className="text-[11px] text-[#8B8BA8]">{tr('ct.sets')} <span className="text-[#B8B8CC] font-semibold">{plantilla.bestOf}</span> {tr('ct.sugeridoPor')} <GameChip juegoId={juego} size={12} /> {tr('ct.editableAdmin')}</p>
          </Field>
        </Section>
        ...
        <Section title={tr('ct.coorgs')}>
          <p className="text-[12px] text-[#8B8BA8] mb-2">{tr('ct.coorgsSub')}</p>
          <div className="flex flex-wrap gap-1.5">
            {Object.values(ORGANIZADORES).filter(o => o.id !== orgId).slice(0, 4).map(o => {
          ...
          {coOrgs.length > 0 && <p className="mt-2 text-[11px] text-[#B6FF3A] font-semibold">{conParams(tr('ct.coorgsAviso'), { n: coOrgs.length + 1 })}</p>}
        </Section>

        {/* Cuándo y dónde */}
        <Section title={tr('ct.cuandoDonde')}>
          <div className="grid grid-cols-2 gap-3">
            <Field label={tr('adm.fecha')}>
          ...
          <Field label={tr('ct.lblSala')}>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => { setSala('local'); if (!sede) setPickerSede(true) }} className={cn('flex items-center justify-center gap-2 h-12 rounded-xl text-sm font-semibold border transition-all',
                ...
                <MapPin size={15} /> {tr('ct.sedeFisica')}
              </button>
                ...
                <div className={`mt-2 card-premium p-3 ${sede.aforo < plazas ? 'border border-[#FF6B6B]/50' : ''}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-[#8B8BA8]">{sede.zona} · {tr('adm.aforo')} <span className="font-mono-num">{sede.aforo}</span> · {sede.setups} setups · <span className="text-[#E0BE63]">★ {sede.rating}</span> · {tr('ml.desde')} <span className="font-mono-num">{precioNocheEfectivo(sede.id, dispoSedes, fichasSede)}€</span>/{tr('ml.noche')}</p>
                    </div>
                    <button onClick={() => setPickerSede(true)} className="h-9 px-3 rounded-lg bg-white/8 text-white text-xs font-bold shrink-0">{tr('ranking.cambiar')}</button>
                  </div>
                  {sede.aforo < plazas && (
                    <p className="mt-2 text-[11px] font-bold text-[#FF8A8A]">{conParams(tr('ct.aforoInsuf'), { aforo: sede.aforo, plazas })}</p>
                  )}
                </div>
              ) : (
          ...
          <Field label={tr('ct.lblCierre')}>
            <div className="flex flex-wrap gap-2">
              {CIERRES.map(c => <Chip key={c} on={cierre === c} onClick={() => setCierre(c)}>{CIERRE_CLAVE[c] ? tr(CIERRE_CLAVE[c]) : c}</Chip>)}
            </div>
          </Field>
        </Section>
        ...
        <Section title={tr('ct.plazasPrecio')}>
          <div className="grid grid-cols-2 gap-3">
            <Field label={tr('adm.plazas')}>
              ...
              {precio === 0 && plazas >= 32 && (
                <p className="mt-1.5 text-[11px] text-[#E0BE63] font-semibold">{tr('ct.gratisTope')}</p>
              )}
              </div>
            </Field>
            <Field label={tr('ges.lblInscripcion')}>
              <div className="flex items-center gap-2">
                <Stepper value={precio} onDec={() => setPrecio(p => { const np = Math.max(0, p - 1); if (np === 0) setPlazas(pl => Math.min(32, pl)); return np })} onInc={() => setPrecio(p => p + 1)} suffix={precio === 0 ? tr('torneo.gratis') : '€'} gratis={precio === 0} />
              </div>
            </Field>
          </div>
          {sala !== 'online' && (
        ...
        <Section title={tr('ct.premios')}>
          ...
          {precio > 0 && (
            <p className="text-[12px] text-[#B8B8CC]">{tr('ct.boteEstimado')} <span className="text-[#B6FF3A] font-bold">{Math.round(plazas * precio * 0.8)}€</span></p>
          )}
        </Section>

        {/* Puntuación: el bracket vive en el motor Torneum y solo Torneum puntúa.
            El tope se calcula solo — el TO lo ve ANTES de publicar. */}
        <Section title={tr('ct.puntuacion')}>
          {(() => {
            const tope = topePuntos({ precio, plazas, online: sala === 'online', bote: precio > 0 ? Math.round(plazas * precio * 0.8) : 0 })
            return (
              <div className="rounded-2xl border border-white/10 bg-white/4 p-3.5">
                <p className="text-sm text-white font-semibold">{tr('ct.repartira')} <span className="text-[#B6FF3A] font-bold font-mono-num">{tope} pts</span> {tr('ct.enElRanking')} {(sala === 'online' ? tr('ranking.online') : tr('ranking.presencial')).toLowerCase()}{tr('ct.leaderboardSuf')}</p>
                <p className="mt-1 text-[11px] leading-relaxed text-[#8B8BA8]">{conParams(tr('ct.topeExplica'), { com: CATEGORIAS.comunidad.corto, ofi: CATEGORIAS.oficial.corto, sm: CATEGORIAS.supermajor.corto })}</p>
              </div>
            )
          })()}
        </Section>
        ...
        <Section title={tr('ct.videoDirecto')}>
          <Field label={tr('ct.lblVideoUrl')}>
            ...
            <p className="text-[11px] text-[#8B8BA8]">{tr('ct.videoAyuda')}</p>
          </Field>
        </Section>

        {/* Reglamento del torneo: lo escribe el TO y el jugador lo ve en un
            desplegable en la ficha (una regla por línea). */}
        <Section title={tr('ct.reglasTitulo')}>
          <Field label={tr('ct.lblReglamento')}>
            <textarea value={reglas} onChange={e => setReglas(e.target.value)} rows={5}
              placeholder={tr('ct.reglasPh')}
            ...
            <p className="text-[11px] text-[#8B8BA8]">{tr('ct.reglasAyuda')}</p>
          </Field>
        </Section>

        {/* Otros comentarios + premios en producto */}
        <Section title={tr('ct.comentarios')}>
          <Field label={tr('ct.lblComentarios')}>
            <textarea value={comentarios} onChange={e => setComentarios(e.target.value)} rows={3}
              placeholder={tr('ct.comentariosPh')}
              className="w-full px-3.5 py-3 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder:text-[#6B6B85] focus:border-[#B6FF3A]/60 outline-none transition-colors resize-none" />
          </Field>
          <Field label={tr('ct.lblPremiosFotos')}>
            ...
            <p className="text-[11px] text-[#8B8BA8] flex items-center gap-1"><ImagePlus size={12} /> {tr('ct.premiosDemo')}</p>
          </Field>
        </Section>
            ...
            {ACCESOS.map(a => {
              ...
              return (
                <button key={a.id} onClick={() => setAcceso(a.id)}
                  ...
                  {a.id !== 'abierto' && <Lock size={12} />} {a.id === 'abierto' ? tr('to.abierto') : a.label}
                </button>
              )
            })}
          </div>
        ...
        {/* Bracket: no se monta aquí — se genera en Gestionar con inscritos reales */}
        <Section title={tr('ct.bracket')}>
          <div className="rounded-2xl border border-white/10 bg-white/4 p-3.5 flex items-start gap-3">
            <span className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-[#B6FF3A]/12 text-[#B6FF3A] shrink-0"><ListTree size={17} /></span>
            <p className="text-[12px] leading-relaxed text-[#B8B8CC]">{tr('ct.bracketExplica')}</p>
          </div>
        </Section>
      </div>
      ...
      {publicado && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
          ...
          <div className="relative w-full max-w-sm card-premium p-6 text-center animate-pop">
            ...
            <p className="mt-4 text-xl font-bold text-white text-display">{tr('ct.publicadoTitulo')}</p>
            <p className="mt-1.5 text-sm text-[#B8B8CC]">«{publicado.nombre}» {tr('ct.publicadoTexto')}</p>
            <div className="mt-5 space-y-2">
              <Link href={`/gestionar/${publicado.id}`} className="w-full h-12 rounded-xl bg-[#B6FF3A] text-[#0A0A0F] font-bold flex items-center justify-center gap-2"><ListTree size={16} /> {tr('ct.gestionarBracket')}</Link>
              <Link href={`/torneo/${publicado.id}`} className="w-full h-12 rounded-xl bg-white/8 text-white font-semibold flex items-center justify-center gap-2"><Eye size={16} /> {tr('ct.verFicha')}</Link>
              <Link href="/explorar" className="w-full h-12 rounded-xl bg-white/8 text-white font-semibold flex items-center justify-center">{tr('ct.irExplorar')}</Link>
            </div>
          </div>
        </div>
      )}
...
// Juego no contemplado en la app: el TO lo PROPONE y el admin lo revisa en su
// panel de Juegos (con plantilla) antes del alta — ya no entra directo al
// catálogo, que es responsabilidad de Torneum.
function NuevoJuegoForm({ onCerrar }: { onCerrar: () => void }) {
  const { t: tr } = useT()
  const proponerJuego = useDemoStore(s => s.proponerJuego)
  const propuestas = useDemoStore(s => s.propuestasJuego)
  const orgId = useOrgId()
  const [nombre, setNombre] = useState('')
  ...
  function proponer() {
    ...
    if (!limpio) return
    const color = COLORES_JUEGO[propuestas.length % COLORES_JUEGO.length]
    proponerJuego({ nombre: limpio, color, emoji, to: organizadorEfectivo(orgId).nombre })
    setEnviado(true)
  ...
  if (enviado) {
    return (
      <div className="mt-2 card-premium p-3.5 flex items-center gap-2.5 border border-[#E0BE63]/35 animate-slide-up-sm">
        <span className="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-[#E0BE63]/12 text-[#E0BE63]"><Check size={16} /></span>
        <p className="text-[12px] text-[#B8B8CC]"><span className="font-bold text-white">{tr('ct.propEnviada')}</span> {conParams(tr('ct.propEnviadaTexto'), { nombre: nombre.trim() })}</p>
      </div>
    )
  }
  ...
  return (
    <div className="mt-2 card-premium p-3.5 space-y-3 animate-slide-up-sm">
      <p className="text-[11px] text-[#8B8BA8]">{tr('ct.propAyuda')}</p>
      <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder={tr('ct.propNombrePh')} autoFocus
      ...
      <button onClick={proponer} disabled={!nombre.trim()}
        className="w-full h-10 rounded-xl bg-[#B6FF3A] text-[#0A0A0F] text-sm font-bold disabled:opacity-40">
        {tr('ct.proponerPre')} «{nombre.trim() || tr('ct.juegoFallback')}» {tr('ct.proponerPost')}
      </button>
    </div>
  )
}
...
// `setupsJuego` viene de la plantilla del juego: si el local no tiene ninguno
// de esos setups se avisa (y se ordenan primero los que sí encajan).
function SedePicker({ plazas, setupsJuego, onClose, onPick }: { plazas: number; setupsJuego: string[]; onClose: () => void; onPick: (l: Local) => void }) {
  const { t: tr } = useT()
  const [q, setQ] = useState('')
  // Precio unificado (dispo publicada > ficha admin > muestra) y sedes
  // suspendidas por el admin fuera de la oferta.
  const dispoSedes = useDemoStore(s => s.dispoSedes)
  const fichasSede = useDemoStore(s => s.fichasSede)
  const tieneSetup = (l: Local) =>
    setupsJuego.length === 0 || l.tiposSetup.some(ts => setupsJuego.includes(ts.toLowerCase().replace('setup stream', 'stream')))
  // Solo se ofrecen sedes cuyo AFORO admite las plazas configuradas del torneo.
  const activas = Object.values(LOCALES).filter(l => !fichasSede[l.id]?.suspendida)
  const conAforo = activas.filter(l => l.aforo >= plazas)
  const excluidas = activas.length - conAforo.length
  const locales = conAforo.filter(l =>
    (l.nombre + ' ' + l.zona).toLowerCase().includes(q.trim().toLowerCase()))
    .sort((a, b) => Number(tieneSetup(b)) - Number(tieneSetup(a)))
  return (
      ...
      <div className="relative w-full max-w-md bg-[#141822] border-t sm:border border-white/10 rounded-t-3xl sm:rounded-3xl animate-slide-up-sm sm:animate-pop max-h-[85vh] overflow-y-auto">
        <div className="sticky top-0 bg-[#141822] px-5 pt-4 pb-3 z-10 border-b border-white/5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[15px] font-bold text-white">{tr('ct.eligeLaSede')}</p>
              <p className="text-[11px] text-[#8B8BA8]">{tr('ct.conAforoPara')} <span className="text-[#B6FF3A] font-bold font-mono-num">{plazas}</span> {tr('ct.jugadores')}{excluidas > 0 ? ` · ${excluidas} ${excluidas === 1 ? tr('ct.localOculto') : tr('ct.localesOcultos')} ${tr('ct.porAforo')}` : ''}</p>
            </div>
          ...
          <div className="mt-3 relative">
            ...
            <input value={q} onChange={e => setQ(e.target.value)} placeholder={tr('ct.buscarLocalPh')} autoFocus
          ...
          {locales.map(l => (
            <button key={l.id} onClick={() => onPick(l)}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-[#8B8BA8]">{l.zona} · {tr('adm.aforo')} <span className="font-mono-num text-white">{l.aforo}</span> · {l.setups} setups · <span className="inline-flex items-center gap-0.5 text-[#E0BE63]"><Star size={9} className="fill-[#E0BE63]" /> {l.rating}</span></p>
              </div>
              <span className="flex flex-col items-end gap-1 shrink-0">
                <span className="text-[11px] font-bold text-[#B6FF3A] font-mono-num">{precioNocheEfectivo(l.id, dispoSedes, fichasSede)}€/{tr('ml.noche')}</span>
                {dispoSedes[l.id]?.publicada && <span className="text-[9px] font-bold uppercase tracking-wide text-[#B6FF3A]/80">{tr('ct.dispoPublicadaTag')}</span>}
                {!tieneSetup(l) && <span className="text-[9px] font-bold uppercase tracking-wide text-[#FF8A5C]">{tr('ct.sinSetup')} {setupsJuego.join('/')}</span>}
              </span>
            </button>
          ))}
          {locales.length === 0 && <p className="text-center text-sm text-[#8B8BA8] py-6">{conParams(tr('ct.sinSedes'), { plazas })}</p>}
          <Link href="/sedes" className="w-full h-11 rounded-xl border border-dashed border-white/20 text-[#B8B8CC] text-sm font-semibold flex items-center justify-center gap-2 hover:text-white transition-colors">
            <MapIcon size={15} /> {tr('ct.descubrirSedes')}
          </Link>
        </div>
      </div>
    </div>
...
function Stepper({ value, onDec, onInc, suffix, icon, gratis }: { value: number; onDec: () => void; onInc: () => void; suffix?: string; icon?: React.ReactNode; gratis?: boolean }) {
  return (
    <div className="flex items-center w-full h-12 bg-white/5 border border-white/10 rounded-xl overflow-hidden">
      ...
      <span className="flex-1 text-center text-white font-bold text-numeric inline-flex items-center justify-center gap-1">{icon}{gratis ? suffix : <>{value}{suffix ? <span className="text-[#8B8BA8] text-sm ml-0.5">{suffix}</span> : ''}</>}</span>
