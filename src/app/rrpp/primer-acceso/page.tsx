...
export default function RrppPrimerAccesoPage() {
  ...
  const [totpActivado, setTotpActivado] = useState(false)
  ...
  // Carga sesión + perfil y decide qué falta.
  useEffect(() => {
    (async () => {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) { router.replace('/rrpp/login'); return }
      const res = await fetch('/api/rrpp/perfil')
      if (!res.ok) { await supabase.auth.signOut(); router.replace('/rrpp/login'); return }
      const { rrpp } = await res.json().catch(() => ({}))
      if (!rrpp) { await supabase.auth.signOut(); router.replace('/rrpp/login'); return }
      if (!rrpp.debe_cambiar_password && rrpp.totp_activado) { router.replace('/rrpp'); return }
      setTotpActivado(!!rrpp.totp_activado)
      setFase(rrpp.debe_cambiar_password ? 'password' : 'totp')
      setCargando(false)
    })()
  }, [router])

  // Genera el QR al entrar en la fase totp (una sola vez).
  const iniciado = useRef(false)
  useEffect(() => {
    if (fase !== 'totp' || totpActivado || iniciado.current) return
    iniciado.current = true
    ;(async () => {
      const res = await fetch('/api/rrpp/cuenta/totp/iniciar', { method: 'POST' })
  ...
  const guardarPassword = async () => {
    ...
    await fetch('/api/rrpp/cuenta/password-cambiada', { method: 'POST' })
    setGuardandoPass(false)
    if (!totpActivado) setFase('totp')
    else finalizar()
  }
  ...
  const confirmarTotp = async () => {
    ...
    setConfirmando(true)
    const res = await fetch('/api/rrpp/cuenta/totp/confirmar', {
  ...
  const finalizar = () => { toast.success('¡Listo! Bienvenido a Torneum'); router.push('/rrpp') }
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-[#0D0F15] p-6">
      ...
      <div className="relative mb-6 text-center">
        ...
        <h1 className="text-display text-2xl font-bold tracking-tight text-white">Configura tu acceso</h1>
        <p className="mt-2 text-sm text-[#A0A0B8]">{fase === 'password' ? 'Elige una contraseña nueva' : 'Protege tu cuenta con un authenticator'}</p>
      </div>
      ...
      {fase === 'password' ? (
        <div className="card-premium relative w-full max-w-sm space-y-4 rounded-3xl p-6 sm:p-8">
          <Input label="Nueva contraseña" type={showPass ? 'text' : 'password'} icon={<Lock size={16} />}
          ...
          <Input label="Repite la contraseña" type={showPass ? 'text' : 'password'} icon={<Lock size={16} />}
        ...
        <div className="card-premium relative w-full max-w-sm space-y-4 rounded-3xl p-6 sm:p-8">
          ...
          <div>
            ...
            <input value={codigo} onChange={e => setCodigo(e.target.value.replace(/\D/g, '').slice(0, 6))}
              onKeyDown={e => { if (e.key === 'Enter') confirmarTotp() }} inputMode="numeric" autoFocus placeholder="000000"
          ...
          <Button fullWidth size="lg" loading={confirmando} onClick={confirmarTotp}><ShieldCheck size={16} /> Activar y entrar</Button>
        </div>
      )}
    </div>
  )
