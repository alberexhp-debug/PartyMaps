...
export default function GestorLoginPage() {
  ...
  const entrar = async () => {
    ...
    if (!gestor) {
      ...
      toast.error('No tienes acceso de Gestor de Torneum')
      setLoading(false)
      return
    }
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center p-6 overflow-hidden bg-[#0D0F15]">
      ...
      <div className="relative mb-8 text-center">
        ...
        <p className="eyebrow mb-2">Equipo Torneum</p>
        <h1 className="text-3xl font-bold text-white text-display tracking-tight">Panel de Gestor</h1>
        <p className="text-[#A0A0B8] text-sm mt-2">Acceso para TorneumGestor</p>
      </div>
