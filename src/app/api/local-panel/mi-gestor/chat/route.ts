...
/**
 * Chat del LOCAL con su TODHGestor (el gestor de TODH que lleva su cartera).
...
export async function POST(req: NextRequest) {
  ...
  const db = createServiceRoleClient()
  const gestorId = await gestorDelLocal(db, t.local_id)
  if (!gestorId) return NextResponse.json({ error: 'Tu local aún no tiene un gestor de TODH asignado.' }, { status: 409 })
