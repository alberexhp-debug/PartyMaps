...
export async function POST(req: NextRequest) {
  ...
  if (previa) {
    ...
    if (previa.receptor_id === yo.id) {
      ...
      enviarPushAUsuario(destino, { title: '¡Nueva amistad!', body: `${yo.nombre} y tú ya sois amigos en Tourneum`, url: '/amigos' }).catch(() => {})
  ...
  enviarPushAUsuario(destino, { title: 'Nueva solicitud de amistad', body: `${yo.nombre} quiere ser tu amigo en Tourneum`, url: '/amigos' }).catch(() => {})
  return NextResponse.json({ ok: true, estado: 'pendiente', nombre: existeDestino.nombre })
}
